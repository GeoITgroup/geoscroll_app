  ...
  Future<void> _check() async {
    ...
    if (mounted) {
      if (tripId != null && deviceId != null) {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => ActiveRideScreen(tripId: tripId, deviceId: deviceId)));
...
class _LoginScreenState extends State<LoginScreen> {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(28), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(width: 72, height: 72,
          decoration: BoxDecoration(gradient: const LinearGradient(colors: [kGreen, kOrange]), borderRadius: BorderRadius.circular(18)),
  ...
  Widget _buildField(TextEditingController ctrl, String hint, IconData icon, {bool obscure = false, Widget? suffix}) {
    return TextField(controller: ctrl, obscureText: obscure, style: const TextStyle(color: Colors.white),
...
class _MainScreenState extends State<MainScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    final screens = [const MapHomeScreen(), const TripsScreen(), const ProfileScreen()];
    return Scaffold(
      body: screens[_tab],
...
class _MapHomeScreenState extends State<MapHomeScreen> {
  ...
  @override
  void initState() { super.initState(); _loadData(); _getLocation(); }
  ...
  List<LatLng> _parseGeofence(dynamic geofence) {
    try {
      if (geofence['coordinates'] != null) {
        ...
        if (coords is List) return coords.map<LatLng>((c) => LatLng(double.parse(c[1].toString()), double.parse(c[0].toString()))).toList();
      }
      ...
      body: Stack(children: [
        FlutterMap(
          ...
          children: [
            ...
            PolygonLayer(polygons: _geofences.map((g) {
              final points = _parseGeofence(g);
              if (points.isEmpty) return null;
              return Polygon(points: points, color: kGreen.withOpacity(0.15), borderColor: kGreen, borderStrokeWidth: 2);
            ...
            MarkerLayer(markers: _scooters.where((s) => s['latitude'] != null && s['longitude'] != null).map((s) {
              ...
              return Marker(
                ...
                width: 56, height: 56,
                child: GestureDetector(onTap: () => _showScooterInfo(context, s),
                  child: Container(width: 44, height: 44,
                    decoration: BoxDecoration(color: available ? kGreen : Colors.grey, shape: BoxShape.circle,
      ...
      builder: (_) => Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(children: [
          Container(width: 56, height: 56,
            decoration: BoxDecoration(color: available ? kGreen.withOpacity(0.1) : Colors.grey.withOpacity(0.1), shape: BoxShape.circle),
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
            decoration: BoxDecoration(color: available ? kGreen.withOpacity(0.1) : Colors.red.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
            child: Text(available ? 'ხელმისაწვდომი' : 'დაკავებული',
        ...
        if (available)
          SizedBox(width: double.infinity, height: 52,
            child: ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: kGreen, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
              onPressed: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => const QRScanScreen())); }))
        else
          Container(padding: const EdgeInsets.all(12),
...
class _QRScanScreenState extends State<QRScanScreen> {
  ...
  void _onDetect(BarcodeCapture capture) {
    ...
    if (value.contains('id=')) {
      deviceId = Uri.parse(value).queryParameters['id'] ?? '';
    } else if (value.contains('device_id=')) {
      deviceId = value.split('device_id=').last;
    } else {
      deviceId = value.trim();
    }
        ...
        Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Stack(children: [
            Container(width: 240, height: 240,
              decoration: BoxDecoration(border: Border.all(color: kGreen, width: 3), borderRadius: BorderRadius.circular(20))),
            Positioned(top: 3, left: 3, right: 3, bottom: 3,
              child: ClipRRect(borderRadius: BorderRadius.circular(17), child: MobileScanner(onDetect: _onDetect))),
          ]),
          const SizedBox(height: 32),
  ...
  Future<void> _startRide() async {
    ...
    try {
      ...
      final res = await http.post(
        Uri.parse('$BASE_URL/api/bog/pay'),
        headers: headers,
      ...
      if (data['success'] == true && data['redirect_url'] != null) {
        ...
        if (tripData['success'] == true && mounted) {
          await prefs.setInt('active_trip_id', tripData['trip_id'] as int);
          await prefs.setString('active_device_id', widget.deviceId);
          Navigator.pushAndRemoveUntil(context,
            MaterialPageRoute(builder: (_) => ActiveRideScreen(tripId: tripData['trip_id'] as int, deviceId: widget.deviceId)),
            (_) => false);
        }
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('შეცდომა: ${e.toString()}')));
    }
          ...
          Container(padding: const EdgeInsets.all(24),
            ...
            child: Column(children: [
              ...
              Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 5),
                decoration: BoxDecoration(color: available ? kGreen.withOpacity(0.1) : Colors.red.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                child: Text(available ? '✅ ხელმისაწვდომია' : '❌ დაკავებულია',
...
class _ActiveRideScreenState extends State<ActiveRideScreen> {
  ...
  Future<void> _endRide() async {
    ...
    try {
      final headers = await _authHeaders();
      final res = await http.post(Uri.parse('$BASE_URL/api/trips/end'), headers: headers,
      ...
      if (data['success'] == true) {
        ...
        if (mounted) showDialog(context: context, barrierDismissible: false, builder: (_) => AlertDialog(
          ...
          actions: [ElevatedButton(
            onPressed: () { Navigator.pop(context); Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const MainScreen()), (_) => false); },
            style: ElevatedButton.styleFrom(backgroundColor: kGreen),
            child: const Text('დახურვა', style: TextStyle(color: Colors.white)))]));
      }
    } catch (_) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('შეცდომა — სცადე ახლიდან'))); }
    setState(() => _ending = false);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: kDark,
      body: SafeArea(child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [
        const SizedBox(height: 20),
        Container(width: 120, height: 120,
          decoration: BoxDecoration(color: kGreen.withOpacity(0.15), shape: BoxShape.circle, border: Border.all(color: kGreen, width: 2)),
        ...
        SizedBox(width: double.infinity, height: 58,
          child: ElevatedButton.icon(
            icon: _ending ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
              : const Icon(Icons.stop_circle, color: Colors.white, size: 24),
            label: Text(_ending ? 'მუშავდება...' : 'გაქირავების დასრულება',
              style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
            ...
            return Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16),
              ...
              child: Row(children: [
                ...
                Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(color: completed ? kGreen.withOpacity(0.1) : kOrange.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                  child: Text(completed ? 'დასრულდა' : (t['status'] ?? '—'),
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() { _username = prefs.getString('username') ?? '—'; _role = prefs.getString('role') ?? 'client'; });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: kBg,
      appBar: AppBar(backgroundColor: kDark, title: const Text('პროფილი', style: TextStyle(color: Colors.white)), automaticallyImplyLeading: false),
        ...
        Container(padding: const EdgeInsets.all(24),
          ...
          child: Column(children: [
            Container(width: 80, height: 80,
              decoration: const BoxDecoration(gradient: LinearGradient(colors: [kGreen, kOrange]), shape: BoxShape.circle),
